...
companion path, but this chapter does not certify its visual QA or the final independent
lesson review.
...
IDs, twelve trace IDs, six figures, and one `LESSON_12_PASS`. The notebook prints the
...
Use this exact nine-slide script. If the deck is unavailable or not yet visually
certified, teach from the case, formula, trace, and diagnosis tables in this chapter.
Do not describe that recovery route as a rendered deck.
...
| 3:45-5:00 | 4 | Explain one versioned case: mission, expected calls, dependencies, evidence, limitations, status, and call budget. |
| 5:00-6:30 | 5 | Map one dataset to pure scorers, one MLflow run per configuration, and one root trace per case. |
| 6:30-8:00 | 6 | Read a root trace from planning through tool attempts, replanning, evidence gate, and report. |
| 8:00-9:30 | 7 | Compare deterministic checks with optional semantic judges. State that absent evidence means `NOT RUN`. |
| 9:30-11:00 | 8 | Diagnose answer-good/path-bad, path-good/answer-incomplete, gate, and citation failures. |
| 11:00-12:00 | 9 | State the deterministic release gate and list the capstone decisions that remain open. |

The rows total 12 minutes. The deck establishes the evaluation model; the notebook
supplies the observed evidence.

## 40-minute notebook route

Run the stable 27-cell notebook from top to bottom. The mapping names every cell and all
six figures.

| Time | Cells | Instructor action | Expected visible output |
|---:|---|---|---|
| 0:00-4:00 | `lesson12-000`, `lesson12-001`, `lesson12-002`, `lesson12-003`, `lesson12-004`, `lesson12-005` | State the public-state and safety boundary, initialize the explicit offline route, verify the case and fixture hashes, and inspect one complete expectation. | Six cases, `agent-cases-v1`, exact SHA-256, two configuration IDs, and Figure 1, **Versioned expectations evaluate trajectory and answer separately**. |
| 4:00-9:00 | `lesson12-006`, `lesson12-007`, `lesson12-008` | Run the real Lesson 11 MCP mission once, compare only its public signature with `reference_completed`, and inspect its plan, typed error, trajectory, gate, and cited facts. | `Reference public signature: MATCH`, five attempts, one `unsupported_metric`, one replan, evidence IDs, sources, and a complete briefing table. |
| 9:00-15:00 | `lesson12-009`, `lesson12-010`, `lesson12-011`, `lesson12-012` | Align dependency-aware call signatures, compute correctness and efficiency, and read the public timeline. | Figure 2, **Expected and observed dependency-aware call signatures align**; both trajectory rationales; Figure 3, **One public trace retains phase, attempt, revision, status, and latency**. |
| 15:00-21:00 | `lesson12-013`, `lesson12-014` | Compute relevance and completeness, then enforce the strict metric fact, document fact, and aggregate sources rules. | Answer rationales, citation-rule table, and `citation_integrity=1.0` for the reference. |
| 21:00-27:00 | `lesson12-015`, `lesson12-016` | Log all six `bounded-agent-v1` cases, then all six `regressed-agent-v0` cases against the identical version and hash. | Two distinct run IDs and six traces per configuration in local SQLite-backed MLflow. |
| 27:00-33:00 | `lesson12-017`, `lesson12-018`, `lesson12-019` | Read means and pass counts, then inspect every case before comparing aggregates. | Twelve score rows, tool and latency tables, failure ownership, Figure 4, **Per-case metrics reveal failures hidden by configuration means**, and Figure 5, **Aligned configurations compare all five means on one dataset hash**. |
| 33:00-37:00 | `lesson12-020`, `lesson12-021` | Contrast one answer-good/path-bad case with one path-good/answer-incomplete case. Load the persisted `bounded-agent-v1/unsupported_metric_not_recovered` root trace, verify its run association, read its ordered phase and tool spans, and assign the earliest public owner. | Two diagnostic rows; a notebook-visible root/child span table with run ID, trace/root IDs, span type, status, phase, attempt, revision, `unsupported_metric`, blocked guardrail, and `evidence_gate` owner; and Figure 6, **Failure diagnosis assigns the earliest public owner**. |
| 37:00-40:00 | `lesson12-022`, `lesson12-023`, `lesson12-024`, `lesson12-025`, `lesson12-026` | Keep both optional judge routes visibly `NOT RUN`, print the local paths and UI command, run deterministic assertions, pose the knowledge check, and state the capstone handoff. | OpenAI/Ollama configuration examples, database and artifact paths, `http://127.0.0.1:5000`, exactly one `LESSON_12_PASS`, challenge constraints, and no selected capstone architecture. |
...
The browser UI can make links convenient, but the notebook-visible tables are the
classroom evidence. A run count without score rows and an inspectable trace is not
sufficient.
